...
import { UserPrisma } from 'src/user/interface/prisma.util';
...
export const socialLoginUtil = async (request: Request, response: Response) => {
  ...
  let isAlreadyEmail = await userPrismaUtil().findByEmail(userEmail);
  ...
  if (!isAlreadyEmail) {
    ...
    isAlreadyEmail = await userPrismaUtil().saveUser(userInfo);
  }
...
const createRandomName = async (name: string): Promise<string> => {
  const isAlreadyName = await userPrismaUtil().findByName(name);
